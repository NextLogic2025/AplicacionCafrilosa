...
import logo from '../assetsProxy/logo'
...
export function LoginScreen({ onSignedIn }: Props) {
  ...
  const onSubmit = handleSubmit(async ({ email, password }) => {
    try {
      ...
      // Persistencia controlada por el usuario (Recordarme).
      if (remember) await SecureStore.setItemAsync('cafrilosa.token', result.token)
      else await SecureStore.deleteItemAsync('cafrilosa.token')
  ...
  return (
    <SafeAreaView className="flex-1 bg-neutral-50">
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1">
        <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ flexGrow: 1, paddingHorizontal: 22, paddingVertical: 26 }}>
          <View className="mx-auto w-full max-w-md">
            <View className="overflow-hidden rounded-[28px] bg-white shadow-2xl shadow-black/20">
              ...
              <View className="px-6 pb-7 pt-6">
                {serverError ? (
                  <Text className="mb-4 rounded-2xl border border-red-300 bg-red-50 px-3 py-2 text-red-800">{serverError}</Text>
                ) : null}
                ...
                <View className="gap-4">
                  ...
                  <PrimaryButton title={isSubmitting ? 'Ingresando…' : 'Iniciar sesión'} loading={isSubmitting} onPress={onSubmit} style={{ width: '100%' }} />
                </View>
              </View>
            </View>
          </View>
