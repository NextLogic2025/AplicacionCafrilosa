...
export function SplashScreen({ onDone }: Props) {
  ...
  React.useEffect(() => {
    // Animación + espera (2s) antes de pasar al login.
    const rotateLoop = Animated.loop(
      Animated.sequence([
        Animated.timing(rotate, { toValue: 1, duration: 950, easing: Easing.inOut(Easing.quad), useNativeDriver: true }),
        Animated.timing(rotate, { toValue: 0, duration: 950, easing: Easing.inOut(Easing.quad), useNativeDriver: true }),
      ])
    )
