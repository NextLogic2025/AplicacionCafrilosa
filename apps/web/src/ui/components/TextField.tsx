...
export const TextField = React.forwardRef<HTMLInputElement, Props>(function TextField(
  ...
  return (
    <label className="grid gap-2">
      ...
      >
        {left ? <span className={cn('shrink-0', isLight ? 'text-neutral-400' : 'text-white/55')}>{left}</span> : null}
        <input
          ref={ref}
          className={cn(
            'w-full min-w-0 flex-1 border-0 bg-transparent outline-none',
            isLight ? 'text-neutral-900 placeholder:text-neutral-400' : 'text-white/90 placeholder:text-white/45'
          )}
          {...inputProps}
        />
        {right ? <span className="shrink-0">{right}</span> : null}
      </span>
      {error ? <span className={cn('text-xs', isLight ? 'text-red-700' : 'text-red-300')}>{error}</span> : null}
    </label>
  )
})
