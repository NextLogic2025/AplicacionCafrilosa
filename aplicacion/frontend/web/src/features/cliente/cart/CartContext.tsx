...
export function CartProvider({
  ...
  const [items, setItems] = React.useState<CartItem[]>(() => loadCart(storageKey))
  ...
  // Actualizar items si cambia la storageKey (ej. cambio de cliente)
  React.useEffect(() => {
  ...
  const syncCartFromServer = React.useCallback(async () => {
    if (!getToken()) return
    try {
      const remote = await getCart(clienteId)
  ...
  const removeItem = React.useCallback(
    ...
    [syncCartFromServer, clienteId, storageKey],
  )
  ...
  const updateQuantity = React.useCallback(
    ...
    [items, removeItem, syncItemQuantity, storageKey],
  )
  ...
  const clearCart = React.useCallback(() => {
    ...
    setLastAction(null)
    if (!getToken()) return
    clearCartRemote(clienteId).catch(() => { })
