	...
	return (
		<div className="space-y-6">
			...
			<div>
				...
				<p className="mt-2 text-gray-600">Gestiona tus facturas e historial de pagos</p>
			</div>
			...
			<div className="flex flex-wrap gap-2">
				{['all', 'PAID', 'PENDING', 'OVERDUE'].map(estado => (
					<button
						key={estado}
						onClick={() => setFiltros({ ...filtros, status: estado as any })}
						className={`rounded-lg px-4 py-2 text-sm font-medium transition ${
							filtros.status === estado ? 'bg-brand-red text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
						}`}
					>
						{estado === 'all' ? 'Todas' : statusConfig[estado as EstadoFactura]?.label}
					</button>
				))}
			</div>
									...
									facturasFiltradas.map(invoice => (
										<tr key={invoice.id} className="border-b border-gray-100 transition hover:bg-gray-50">
											...
											<td className="px-4 py-3 text-sm">
												<span className={`rounded px-2 py-1 text-xs font-medium ${statusConfig[invoice.status].color}`}>
						...
						<div className="space-y-6 p-6">
							<div className="grid grid-cols-2 gap-4">
								...
								<div>
									...
									<p className={`font-semibold ${statusConfig[facturaSeleccionada.status].color}`}>
										{statusConfig[facturaSeleccionada.status].label}
									</p>
								</div>
								<div>
