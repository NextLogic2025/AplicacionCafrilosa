...
import { MapPin, Phone, Clock, AlertCircle, Eye } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-2 text-gray-600">Rastrea el estado de tus entregas</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-3">
          ...
          <p className="mt-1 text-2xl font-bold text-yellow-900">
            {entregasConEstado.filter(d => d.currentStatus === 'pending').length}
          </p>
        </div>
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-3">
          <p className="text-xs font-medium text-blue-700">En tránsito</p>
          <p className="mt-1 text-2xl font-bold text-blue-900">
            {entregasConEstado.filter(d => d.currentStatus === 'in_transit').length}
          </p>
        </div>
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3">
          <p className="text-xs font-medium text-emerald-700">Entregadas</p>
          <p className="mt-1 text-2xl font-bold text-emerald-900">
            {entregasConEstado.filter(d => d.currentStatus === 'delivered').length}
          </p>
        </div>
        <div className="rounded-lg border border-red-200 bg-red-50 p-3">
          <p className="text-xs font-medium text-brand-red">Problemas</p>
          <p className="mt-1 text-2xl font-bold text-brand-red">
            {entregasConEstado.filter(d => d.currentStatus === 'issue').length}
          </p>
        </div>
      </div>
