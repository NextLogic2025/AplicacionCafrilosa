    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-600">Mantente actualizado con tus notificaciones</p>
        </div>
        {noLeidas > 0 && (
          <button
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-lg border border-red-200 bg-red-50 p-3">
          <p className="text-xs font-medium text-brand-red">No leídas</p>
          <p className="mt-1 text-2xl font-bold text-brand-red">{noLeidas}</p>
        </div>
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-3">
          <p className="text-xs font-medium text-blue-700">Total</p>
          <p className="mt-1 text-2xl font-bold text-blue-900">{notificaciones.length}</p>
        </div>
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
          <p className="text-xs font-medium text-gray-700">Esta semana</p>
      ...
      <div className="flex flex-wrap gap-2">
        <div className="flex gap-1">
          <button
            onClick={() => setFiltros({ ...filtros, read: 'all' })}
            className={`rounded-lg px-3 py-1 text-sm font-medium transition ${
              filtros.read === 'all' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            }`}
          >
            Todas
          </button>
          <button
            onClick={() => setFiltros({ ...filtros, read: 'unread' })}
            className={`rounded-lg px-3 py-1 text-sm font-medium transition ${
              filtros.read === 'unread' ? 'bg-brand-red text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            }`}
          >
            No leídas ({noLeidas})
          </button>
          <button
            onClick={() => setFiltros({ ...filtros, read: 'read' })}
            className={`rounded-lg px-3 py-1 text-sm font-medium transition ${
              filtros.read === 'read' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            }`}
          >
            Leídas
          </button>
