...
export default function PaginaPedidos() {
	...
	const colorEstado = (estado: EstadoPedido) => {
	...
	return (
		<div className="mx-auto max-w-6xl p-4 md:p-8">
			...
			<div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
				<div>
					<h1 className="mb-1 text-2xl font-bold text-gray-900 md:text-3xl">Mis Pedidos</h1>
					<p className="text-gray-600">Gestiona y consulta tus pedidos</p>
				</div>
				<button
			...
			{cargando && !pedidos.length ? (
				...
				<>
									...
									<tr key={pedido.id} className="border-b border-gray-100 hover:bg-gray-50">
										...
										<td className="px-4 py-3">
											<span
												...
												style={{ backgroundColor: colorEstado(pedido.status) }}
											>
					...
					{pedidosTotalPaginas > 1 && (
						<div className="mt-6 flex justify-center gap-2">
							{Array.from({ length: pedidosTotalPaginas }).map((_, i) => (
								<button
									key={i + 1}
									onClick={() => cambiarPagina(i + 1)}
									className={`rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
										paginaActual === i + 1 ? 'text-white' : 'text-gray-600 hover:bg-gray-100'
									}`}
									style={paginaActual === i + 1 ? { backgroundColor: COLORES_MARCA.red } : {}}
								>
									{i + 1}
								</button>
							))}
