...
export function SupervisorCatalogScreen() {
    ...
    const renderProductItem = (item: Product) => {
        ...
        return (
            <TouchableOpacity className="bg-white p-4 mb-3 rounded-2xl shadow-sm border border-neutral-100" activeOpacity={0.7} onPress={() => (navigation as any).navigate('SupervisorProductDetail', { productId: item.id })}>
                        ...
                        <View className="flex-row items-center justify-between mt-1">
                            {item.precios && item.precios.length > 0 ? (
                                <View className="flex-row items-center flex-1">
                                    ...
                                    <Text className="text-neutral-500 text-xs ml-1">{`${item.precios.length} lista${item.precios.length > 1 ? 's' : ''} de precio`}</Text>
                                </View>
                            ) : (
                            ...
                            {activePromosCount > 0 && (
                                <View className="bg-red-100 px-2 py-1 rounded-md ml-2">
                                    <Text className="text-red-600 text-[10px] font-bold">{`${activePromosCount} PROMO${activePromosCount > 1 ? 'S' : ''}`}</Text>
                                </View>
                            )}
                        </View>
                        {hasPromotion && item.precio_oferta && (
                            <View className="flex-row items-center mt-1">
                                <Text className="text-neutral-400 text-xs line-through mr-2">{new Intl.NumberFormat('es-CO',{style:'currency',currency:'COP',minimumFractionDigits:0,maximumFractionDigits:0}).format(item.precio_original || 0)}</Text>
                                <Text className="text-red-600 font-bold text-sm">{new Intl.NumberFormat('es-CO',{style:'currency',currency:'COP',minimumFractionDigits:0,maximumFractionDigits:0}).format(item.precio_oferta)}</Text>
                            </View>
                        )}
        ...
        <View className="flex-1 bg-neutral-50 relative">
            ...
            <View className="bg-white shadow-sm z-10 pb-2">
                <View className="px-5 py-4 flex-row items-center">
                    ...
                    <TouchableOpacity className="w-12 h-12 rounded-xl items-center justify-center shadow-lg bg-red-600" onPress={() => (navigation as any).navigate('SupervisorProductForm')}>
            ...
            <View className="flex-1 px-5 mt-4">
                <GenericList items={filteredProducts} isLoading={loading} onRefresh={fetchData} renderItem={renderProductItem} emptyState={{ icon: 'cube-outline', title: 'Sin Productos', message: 'No hay productos que coincidan con los filtros.' }} />
            </View>
            <ExpandableFab actions={[{ icon: 'pricetags', label: 'Listas de Precio', onPress: () => (navigation as any).navigate('SupervisorPriceLists') }, { icon: 'grid', label: 'Categorías', onPress: () => (navigation as any).navigate('SupervisorCategories') }]} />
        </View>
    )
}
