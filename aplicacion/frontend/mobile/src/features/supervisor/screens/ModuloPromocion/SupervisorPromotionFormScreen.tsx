...
export function SupervisorPromotionFormScreen(props: any) {
    ...
    const loadData = async () => {
        try {
            ...
            if (isEditing && campaign) {
                ...
                const hydratedProducts = rawProducts.map(rp => rp)
    ...
    const renderInputSection = () => (
        <View className="bg-white mx-4 mt-4 p-4 rounded-xl shadow-sm border border-gray-100">
            ...
            <TextInput className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-gray-800" value={nombre} onChangeText={setNombre} placeholder="Ej. Descuento Verano" />
            <View className="flex-row mt-4">
                <View className="flex-1 mr-2">
                    <Text className="text-gray-800 font-bold mb-1 text-sm">Inicio</Text>
                    <TouchableOpacity onPress={() => openDatePicker('start')} className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex-row items-center justify-between">
                ...
                <View className="flex-1 ml-2">
                    ...
                    <TouchableOpacity onPress={() => openDatePicker('end')} className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex-row items-center justify-between">
            ...
            <View className="mt-4">
                ...
                <View className="flex-row h-12">
                    <View className="flex-1 flex-row bg-gray-100 rounded-lg p-1 mr-2">
                        <TouchableOpacity onPress={() => setTipoDescuento('PORCENTAJE')} style={{ flex: 1, justifyContent: 'center', alignItems: 'center', borderRadius: 6, backgroundColor: tipoDescuento === 'PORCENTAJE' ? 'white' : 'transparent' }}>
                            <Text className={`font-bold ${tipoDescuento === 'PORCENTAJE' ? 'text-red-600' : 'text-gray-500'}`}>%</Text>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => setTipoDescuento('MONTO_FIJO')} style={{ flex: 1, justifyContent: 'center', alignItems: 'center', borderRadius: 6, backgroundColor: tipoDescuento === 'MONTO_FIJO' ? 'white' : 'transparent' }}>
                            <Text className={`font-bold ${tipoDescuento === 'MONTO_FIJO' ? 'text-red-600' : 'text-gray-500'}`}>$</Text>
                        </TouchableOpacity>
                    </View>
                    <TextInput className="flex-1 bg-white border border-gray-200 rounded-lg px-4 text-right font-bold text-lg text-red-600" value={valorDescuento} onChangeText={setValorDescuento} keyboardType="numeric" placeholder="0" />
                </View>
            </View>
            <View className="mt-4">
                <Text className="text-gray-800 font-bold mb-1 text-sm">URL Banner (Opcional)</Text>
                <TextInput className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-gray-800" value={imagenBanner} onChangeText={setImagenBanner} placeholder="https://..." />
            </View>
            <View className="flex-row justify-between items-center mt-6 bg-gray-50 p-3 rounded-lg border border-gray-100">
                ...
                <Switch trackColor={{ false: '#767577', true: '#bbf7d0' }} thumbColor={activo ? '#16a34a' : '#f4f3f4'} ios_backgroundColor="#3e3e3e" onValueChange={setActivo} value={activo} />
            </View>
        </View>
    )
        ...
        <View className="bg-white mx-4 mt-4 p-4 rounded-xl shadow-sm border border-gray-100 mb-6">
            ...
            <View className="flex-row mb-4">
                {[
                    ...
                    <TouchableOpacity key={opt.id} onPress={() => setAlcance(opt.id as any)} className={`flex-1 p-3 rounded-xl items-center border ${idx !== 0 ? 'ml-2' : ''} ${alcance === opt.id ? 'bg-red-50 border-red-500' : 'bg-white border-gray-200'}`}>
            ...
            {alcance === 'GLOBAL' && (<View style={{ height: 10 }} />)}
            {alcance === 'POR_LISTA' && (
                <View>
                    <Text className="text-xs font-bold text-gray-400 mb-2">Selecciona Lista de Precios:</Text>
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        {priceLists.map(l => (
                            <TouchableOpacity key={l.id} onPress={() => setListaId(l.id)} className={`mr-2 px-4 py-2 rounded-full border ${listaId === l.id ? 'bg-teal-600 border-teal-600' : 'bg-gray-50 border-gray-200'}`}>
            ...
            {alcance === 'POR_CLIENTE' && (
                <View>
                    <View className="flex-row justify-between items-center mb-3 mt-2">
                        ...
                        <TouchableOpacity onPress={() => setPickerType('clients')} className="bg-red-600 px-4 py-2 rounded-lg flex-row items-center">
                            ...
                            {promoClients.map(c => (
                                <View key={c.cliente_id} className="bg-white rounded-lg p-3 border border-neutral-100 flex-row items-center mb-2">
                                    ...
                                    <View className="flex-1">
                                        <Text className="text-sm text-neutral-900 font-semibold">{c.cliente?.razon_social || c.cliente?.nombre_comercial || `ID: ${c.cliente_id}`}</Text>
                                        {c.cliente?.identificacion && (
                                            <Text className="text-xs text-neutral-500 mt-0.5">{c.cliente.identificacion}</Text>
                                        )}
                                    </View>
                                    <TouchableOpacity onPress={() => removeItem(c.cliente_id, 'client')} className="w-8 h-8 rounded-full bg-red-100 items-center justify-center">
        ...
        return (
            <View className="bg-white mx-4 mt-0 mb-8 p-4 rounded-xl shadow-sm border border-gray-100">
                <View className="flex-row justify-between items-center mb-4">
                    ...
                    <TouchableOpacity onPress={() => setPickerType('products')} className="bg-red-600 px-3 py-1.5 rounded-full flex-row items-center">
                    ...
                    promoProducts.map(p => {
                        ...
                        return (
                            <View key={p.producto_id} className="mb-4 pb-4 border-b border-gray-100">
                                <TouchableOpacity onPress={() => hasPrices && toggleProductExpansion(p.producto_id)} className="flex-row items-center mb-3" activeOpacity={hasPrices ? 0.7 : 1}>
                                        ...
                                        <View className="flex-row items-center">
                                            ...
                                            {hasPrices && (
                                                <View className="flex-row items-center ml-2">
                                                    ...
                                                    <Text className="text-[10px] text-green-600 ml-1">{producto.precios?.length || 0} {producto.precios?.length !== 1 ? 'listas' : 'lista'}</Text>
                                                </View>
                                            )}
                                        </View>
                                    </View>
                                ...
                                {hasPrices ? (
                                    isExpanded && (
                                        <View className="ml-2">
                                            <ProductPriceDisplay precios={producto.precios} priceLists={priceLists} showAllPrices={true} precioOfertaFijo={p.precio_oferta_fijo} tipoDescuento={tipoDescuento} valorDescuento={Number(valorDescuento) || undefined} />
                                        </View>
                                    )
                                ) : (
                                    <View className="ml-2 bg-amber-50 p-4 rounded-lg border border-amber-200">
                                        <View className="flex-row items-center">
                                            ...
                                            <View className="flex-1 ml-3">
                                                ...
                                                <Text className="text-amber-600 text-[10px]">Este producto necesita precios asignados para calcular descuentos. Ve a Catálogo → Listas de Precios para configurarlos.</Text>
                                            </View>
                                        </View>
                                    </View>
                                )}
            ...
            <View className="flex-1 bg-neutral-50">
                <View className="pt-12 px-4 pb-4 bg-white border-b border-neutral-200 shadow-sm">
                    <View className="flex-row items-center mb-3">
                        <TouchableOpacity onPress={() => { setPickerType('none'); setSearchText('') }} className="w-10 h-10 items-center justify-center rounded-full bg-neutral-100 mr-3">
                    ...
                    <View className="flex-row items-center bg-neutral-100 rounded-xl px-4 h-12">
                        ...
                        <TextInput className="flex-1 ml-2 text-base text-neutral-900" placeholder={isClient ? "Buscar por nombre o identificación..." : "Buscar por nombre o SKU..."} placeholderTextColor="#9CA3AF" value={searchText} onChangeText={setSearchText} autoFocus />
                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 16 }}>
                    {filtered.map((item, index) => (
                        <TouchableOpacity key={item.id} onPress={() => addItem(item)} className="bg-white rounded-xl p-4 mb-3 shadow-sm border border-neutral-100" activeOpacity={0.7}>
                            <View className="flex-row items-center">
                                ...
                                <View className="flex-1">
                                    <Text className="font-bold text-neutral-900 text-base mb-1">{isClient ? (item.razon_social || item.nombre) : item.nombre}</Text>
                                    <View className="flex-row items-center">
                                        <Ionicons name="pricetag-outline" size={12} color="#9CA3AF" />
                                        <Text className="text-sm text-neutral-500 ml-1">{isClient ? item.identificacion : item.codigo_sku}</Text>
                                    </View>
                                    {isClient && item.ciudad && (<View className="flex-row items-center mt-1"><Ionicons name="location-outline" size={12} color="#9CA3AF" /><Text className="text-xs text-neutral-400 ml-1">{item.ciudad}</Text></View>)}
                                </View>
    ...
    return (
        <View className="flex-1 bg-gray-50">
            ...
            <FeedbackModal visible={feedbackModal.visible} type={feedbackModal.type} title={feedbackModal.title} message={feedbackModal.message} onClose={closeModal} onConfirm={feedbackModal.onConfirm} showCancel={feedbackModal.showCancel} confirmText={feedbackModal.confirmText} />
        </View>
    )
}
